...
def points_per_metal_layer(net_list: List[Net]) -> Tuple[dict,dict]:
    ...
    for net in net_list: 
        ...
        for element in net.routing_elements: 
            ...
            metal_layer_mapping[element.metal_layer].append((net,
                element.starting_point.first_coordinate,
                element.starting_point.second_coordinate))
            
            if element.ending_point:
                metal_layer_mapping[element.metal_layer].append((net,
                    element.ending_point.first_coordinate,
                    element.ending_point.second_coordinate))
...
def find_minimum_distance_across_layers(net: Net, KDtrees: Dict[str,KDTree], layer_mapping: dict) -> Tuple[Net,str]:
    """ 
    >>> net_a = Net("net_a", [], [
    ...     RoutingElement('metal1', 'via2', RoutingPoint(0, 100), RoutingPoint(100, '*')),
    ...     RoutingElement('metal1', None, RoutingPoint(100, 100), RoutingPoint(200, '*')),
    ...
    for routing_element in net.routing_elements:
        ...
        distances, indices = tree.query(np.array([(
                routing_element.starting_point.first_coordinate,
                routing_element.starting_point.second_coordinate)]),
            k=min(100,len(layer_mapping[routing_element.metal_layer])))
        ...
        if routing_element.ending_point:
            ...
            distances, indices = tree.query(np.array([(
                routing_element.ending_point.first_coordinate,
                routing_element.ending_point.second_coordinate)]),
                k=min(100,len(layer_mapping[routing_element.metal_layer])))
    ...
    assert(minimum_distance_net_layer), "Net layer is None"
...
if __name__=="__main__":
    ...
    param_parser.add_argument('-m', "--grouping_mode", action = "store", help = "Net grouping approach", default = "accurate", choices = ["accurate","relaxed"])   
